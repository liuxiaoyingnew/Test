...
public class LoginPageHelper {
	...
	public static void waitPageLoad(SeleniumUtil seleniumUtil,int timeOut){
	    ...
	    By []by = new By[]{LoginPage.HP_TYPE_NAME,LoginPage.HP_TYPE_PWD};
		for(int i = 0;i<2;i++){
		   seleniumUtil.waitForElementToLoad(timeOut, by[i]);
		}		
		logger.info("元素加载完毕");		
	}
	public static void waitDetailPageLoad(SeleniumUtil seleniumUtil,int timeOut){
	    logger.info("开始等待元素加载");
	    By []by = new By[]{LoginPage.HP_TYPE_NAME,LoginPage.HP_TYPE_PWD};
		for(int i = 0;i<2;i++){
		   seleniumUtil.waitForElementToLoad(timeOut, by[i]);
	 ...
	 * 检查元素是否存在
	 */
	public static boolean isHomePageElements(SeleniumUtil seleniumUtil,By by){
